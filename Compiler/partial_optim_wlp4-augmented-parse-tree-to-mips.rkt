...
(define (get-value-node node)
  ;(println node)
  (get-value-str (v-obj-val (node-vobj node))))

(define (get-root-node node)
  (get-root-str (v-obj-val (node-vobj node))))
...
(define (reduce-uesless-declarations func-node params)
  ; Building Table for Used and Modified Variables
  (define used-variable-table (make-hash))
  (define constant-variable-table (make-hash))
  ;(define modified-variable-table (make-hash))

  ;UTILITY FUNCTIONS
  (define (resolve-to-constant? expr)
    (define to-constant #t)
    (define (resolve-to-constant-helper1 node)
      (define (resolve-to-constant-helper2 expr-tree)
        (define nlst (node-lst expr-tree))
        (define val-list (string-split (v-obj-val (node-vobj expr-tree))))
        
        (define (match-list lst)
          (for [(nl lst)]
            (resolve-to-constant-helper1 nl)))
     
        (match val-list
          [(list "factor" "ID")
           (set! to-constant #f)]
          [_ (match-list nlst)]))
      (resolve-to-constant-helper2 node)
      (void))
    (resolve-to-constant-helper1 expr)
    ;(displayln "Expersion:")
    ;(displayln expr)
    ;(printf "Result: ~a" to-constant)
    to-constant)
    

  (define (grab-dcl-id dcl-tree)
    ;(println dcl-tree)
    (get-value-node (list-ref (node-lst dcl-tree) 1)))

  (define gli-lvalue-id "")

  (define (grab-lvalue-id lval)
    (define (grab-lvalue-id-h lval-tree)
      (define root (get-root-node lval-tree))
      (define nlst (node-lst lval-tree))
      ;(printf "ROOT is at : ~a \n" root)
      (match root
        ["ID" (set! gli-lvalue-id (get-value-node lval-tree))]
        [_ (for [(node nlst)]
             (grab-lvalue-id node))]
        ))
    (grab-lvalue-id-h lval)
    gli-lvalue-id)

  (define (constant-var? variable)
    (if (verify-in-table constant-variable-table variable)
        (car (hash-ref constant-variable-table variable))
        #f))
  
  (define (used-var? variable)
    ;(print used-variable-table)
    (if (verify-in-table used-variable-table variable)
        (hash-ref used-variable-table variable)
        #f))
  
  (define (import-useless-variables p-dcl-table)
    (hash-for-each
     p-dcl-table
     (lambda (key val)
       ; TODO: REMOVE THIS USELESS CONDITION
       ;(if #t
           (begin
             (hash-set! used-variable-table (dcl-obj-symbol key) #f)
             (hash-set! constant-variable-table (dcl-obj-symbol key) (list #f empty)))
           (void))
       ;)
    ))

  (define (recurse-on-lst-used lst)
    (for [(node lst)]
      (build-used-table node)))
  
  (define (build-used-table node)
    ;(println "Running Build Used Table")
    (define val-list (string-split (v-obj-val (node-vobj node))))
    ;(println val-list)
    (define nlst (node-lst node))
    (match val-list
      ;[empty (void)]
      [(list "factor" "ID")
       (define USED-ID (get-value-node (list-ref nlst 0)))
       ;(printf "USED-ID IN BUILD USED TABLE: ~a \n" USED-ID)
       (hash-set! used-variable-table USED-ID #t)
       ]
      [(list "lvalue" "ID")
       (define USED-ID (get-value-node (list-ref nlst 0)))
       ;(printf "USED-ID IN BUILD USED TABLE: ~a \n" USED-ID)
      ...
      [_ (recurse-on-lst-used (node-lst node))]))

  (define (verify-in-table table key)
    (hash-has-key? table key))
  
  (define (recurse-on-lst-useless lst)
    (for [(node lst)]
      (build-useless-table-helper node)))

  (define (build-useless-table-helper node)
    ;(println "Running build-useless-table-helper")
    (define val-list (string-split (v-obj-val (node-vobj node))))
    (define rlst (reverse (node-lst node)))
    (define nlst (node-lst node))
    ;(println val-list)
    (match val-list
      [(list "factor" "ID")
      ...
      [(list "statement" "lvalue" "BECOMES" "expr" "SEMI")
       (define lval-id (grab-lvalue-id (list-ref nlst 0)))
       ;(if (equal? (get-value-node (list-ref nlst 0)) "ID")
       (if (used-var? lval-id)
           (if (constant-var? lval-id)
               (void)
               (if (resolve-to-constant? (list-ref nlst 2))
                   (begin
                     (hash-set! constant-variable-table
                                (grab-lvalue-id (list-ref nlst 0))
                                (list #t (list-ref nlst 2)))
                     ;(println "++++++++++++++++++++++++++++++++++++++++++++++++++")
                     ;(println constant-variable-table)
                     ;(println "++++++++++++++++++++++++++++++++++++++++++++++++++")
                     )
                   (build-used-table (list-ref nlst 2))))
           (if ()))]
      [(list "main" "INT" "WAIN" "LPAREN" "dcl" "COMMA" "dcl" "RPAREN"
                 "LBRACE" "dcls" "statements" "RETURN" "expr" "SEMI" "RBRACE")
       (build-used-table (list-ref nlst 11))
       (build-useless-table-helper (list-ref nlst 9))
       ]
      ;[empty (void)]
      [(list "procedure" "INT" "ID" "LPAREN" "params" "RPAREN"
                 "LBRACE" "dcls" "statements" "RETURN" "expr" "SEMI" "RBRACE")
       ;(println "Procedure Build Used Variable Table")
       (build-used-table (list-ref nlst 9))
       ;(println used-variable-table)
       (build-useless-table-helper (list-ref nlst 7))
       ]
      [_ (recurse-on-lst-useless rlst)]
    ))

  ; Reducing Parse Tree to Remove/Reduce Unused and Unmodified Variables
  (define purely-useless-variable-table (make-hash))
  (define reducible-variable-table (make-hash))
  (define (post-process-variable-table)
    (hash-for-each
     used-variable-table
     (lambda (key val)
       (begin
         (if val
           (if (constant-var? key)
               (hash-set! reducible-variable-table key (cadr (hash-ref constant-variable-table key)))
               (void))
           (hash-set! purely-useless-variable-table key val))))))

  (define (purely-useless? var)
    (define uselessness (hash-has-key? purely-useless-variable-table var))
    ;(printf "Checking ~a against ~a. Should return Useless = ~a \n" var purely-useless-variable-table t)
    uselessness)


  ; Helper Function: Checking if ID is inside expr tree
  (define (expr-id-checker expr-tree id)
    (define truth #f)
    (define (recurse-on-expr-lst lst)
      (for [(node lst)]
        (check-expr-helper node)))
  
    (define (check-expr-helper node)
      ;(println "Running Build Used Table")
      (define val-list (string-split (v-obj-val (node-vobj node))))
      ;(println val-list)
      (define nlst (node-lst node))
      (match val-list
        [(list "factor" "ID")
         (define SEEN_ID (get-value-node (list-ref nlst 0)))
         (if (equal? id SEEN_ID)
             (set! truth #t)
             (void))
         ]
        ...
        [_ (recurse-on-expr-lst nlst)]))
    (check-expr-helper expr-tree)
    truth
    )

  (define (specific-statement-has-useless-id statement-tree)
    (define truth-list empty)
    (define (ss-helper st-tree id)
      (define val-list (string-split (v-obj-val (node-vobj statement-tree))))
      (define nlst (node-lst statement-tree))
      ;(println "IN SPECIFIC STATEMENT FUNCTION 1")
      ;(println val-list)
      (match val-list
        [(list "statement" "lvalue" "BECOMES" "expr" "SEMI")
         (define t1 (expr-id-checker (list-ref nlst 0) id))
         (define t2 (expr-id-checker (list-ref nlst 2) id))
         ;(println nlst)
         ;(printf "Truth (lvalue expr): ~a    ~a \n" t1 t2)
         (set! truth-list (cons (or t1 t2) truth-list))
         ]
        [_ (print-end "ERROR: You should not be searching for useless ids this statement")]
        ))
    (for [((id val) purely-useless-variable-table)]
      (ss-helper statement-tree id)
      )
    (define truth (ormap (lambda (x) x) truth-list))
    ;(println truth-list)
    truth
    )
  
  ; Remove any non-used declarations in dcls, then remove any non-used in any of the statement lvalue becomes expr semi
  (define (remove-useless-vars var-node)
    ;(printf "----------------------- ~a ---------------------- \n" (func-sign-name current-scope))
    ; This returns a new dcl tree
    (define (remove-useless-dcl-tree dcl-tree)
      (define val-list (string-split (v-obj-val (node-vobj dcl-tree))))
      (define nlst (node-lst dcl-tree))
      (define (build-new-dcl-tree-from-lst tree-lst result)
        (if (empty? tree-lst)
            (reverse result)
            (build-new-dcl-tree-from-lst
             (cdr tree-lst)
             (cons (remove-useless-dcl-tree (car tree-lst)) result))))
      (match val-list
        [(list "dcls") dcl-tree]
        [(list "dcls" "dcls" "dcl" "BECOMES" "NUM" "SEMI")
         (if (not (purely-useless? (grab-dcl-id (list-ref nlst 1))))
             (begin
               ;(printf "Used var: ~a  in  ~a \n" (grab-dcl-id (list-ref nlst 1)) (func-sign-name current-scope))
               (node
                (node-vobj dcl-tree)
                (build-new-dcl-tree-from-lst (node-lst dcl-tree) empty)))
             (begin
               ;(printf "Useless var: ~a  in  ~a \n" (grab-dcl-id (list-ref nlst 1)) (func-sign-name current-scope))
               (remove-useless-dcl-tree (list-ref nlst 0))))
         ]
        [(list "dcls" "dcls" "dcl" "BECOMES" "NULL" "SEMI")
         (if (not (purely-useless? (grab-dcl-id (list-ref nlst 1))))
             (begin
               ;(printf "Used var: ~a  in  ~a \n" (grab-dcl-id (list-ref nlst 1)) (func-sign-name current-scope))
               (node
    ...
    (define (remove-useless-statement-tree st-tree)
      ;(println st-tree)
      (define val-list (string-split (v-obj-val (node-vobj st-tree))))
      ;(println "Calling remove-useless-statement-tree Function")
      ;(println val-list)
      (define nlst (node-lst st-tree))
      (match val-list
        [(list "statements")
         ;(println "Singleton Statement")
         st-tree
         ]
        [(list "statements" "statements" "statement")
         ;Reducing
         (define sub-statement-val (v-obj-val (node-vobj (list-ref nlst 1))))
         ;(println "The Statement Type is currently statements statements statement") 
         ;(println sub-statement-val)
         (match sub-statement-val
           ["statement lvalue BECOMES expr SEMI"
            ;(println "Next Statement is Assignment Type")
            (if (specific-statement-has-useless-id (list-ref nlst 1))
                (begin
                  ;(println "Removed Statements")
                  (remove-useless-statement-tree (list-ref nlst 0)))
                (begin
                  ;(println "Statement is being used")
                  (node
                   (node-vobj st-tree)
                   (list
                    (remove-useless-statement-tree (list-ref nlst 0))
           ...
           [_
            ;(println "Next Statement is Normal Statement")
            (node
             (node-vobj st-tree)
             (list
              (remove-useless-statement-tree (list-ref nlst 0))
              (remove-useless-statement-tree (list-ref nlst 1))
              ))
            ])
         ]
        ;[(list "statement" "lvalue" "BECOMES" "expr" "SEMI")]
        [(list "statement" "IF" "LPAREN" "test" "RPAREN" "LBRACE" "statements" "RBRACE" "ELSE" "LBRACE" "statements" "RBRACE")
         (node
          ...
          (list
           ...
           (list-ref nlst 3)
           (list-ref nlst 4)
           (remove-useless-statement-tree (list-ref nlst 5)) ;statements
           (list-ref nlst 6)
           (list-ref nlst 7)
           (list-ref nlst 8)
           (remove-useless-statement-tree (list-ref nlst 9)) ;statements
           (list-ref nlst 10)
           ))
         ]
        ...
        ; For now, we have not optimized print / delete optimization
        [(list "statement" "PRINTLN" "LPAREN" "expr" "RPAREN" "SEMI") st-tree]
        [(list "statement" "DELETE" "LBRACK" "RBRACK" "expr" "SEMI") st-tree]
        [_
         ;(println val-list)
         (print-end "ERROR: You Should Never Be Here Reducing Statement!")
         ]
      ))
    
    (define (reduce-var-tree tree)
      (define val-list (string-split (v-obj-val (node-vobj tree))))
      ;(displayln "Starting To Reduce New Function")
      (define nlst (node-lst tree))
      (define (build-reduced-tree tree-lst result)
        (if (empty? tree-lst)
          (reverse result)
          (build-reduced-tree
           (cdr tree-lst)
           (cons (reduce-var-tree (car tree-lst)) result))))
      (define (build-reduce-tree-from-node t-node)
        (match val-list
          [(list "main" "INT" "WAIN" "LPAREN" "dcl" "COMMA" "dcl" "RPAREN"
                 "LBRACE" "dcls" "statements" "RETURN" "expr" "SEMI" "RBRACE")
           (node
            ...
            (list
             ...
             (list-ref nlst 6)
             (list-ref nlst 7)
             (remove-useless-dcl-tree (list-ref nlst 8)) ;This is dcls
             (remove-useless-statement-tree (list-ref nlst 9)) ;This is statements
             (list-ref nlst 10)
             (list-ref nlst 11) ;This is expr
             (list-ref nlst 12)
             (list-ref nlst 13)
             ))
           ]
          [(list "procedure" "INT" "ID" "LPAREN" "params" "RPAREN"
           ...
           (node
            ...
            (list
             ...
             (list-ref nlst 4)
             (list-ref nlst 5)
             (remove-useless-dcl-tree (list-ref nlst 6)) ;This is dcls
             (remove-useless-statement-tree (list-ref nlst 7)) ;This is statements
             (list-ref nlst 8)
             (list-ref nlst 9) ;This is expr
             (list-ref nlst 10)
             (list-ref nlst 11) 
             ))
           ]
          [_
           (node
            (node-vobj t-node)
            (build-reduced-tree (node-lst t-node) empty))]
          ))
      (build-reduce-tree-from-node tree))
    (reduce-var-tree var-node))

  (define proc-dcl-table (hash-ref func-sym-table current-scope))
  (import-useless-variables proc-dcl-table)
  (build-useless-table-helper func-node)
  (post-process-variable-table)

#|
  ; DEBUG PURPOSE
  (printf "\n\n....................... ~a ...................... \n\n" (func-sign-name current-scope))
  (displayln "-------------------- Used -----------------------")
  (displayln used-variable-table)
  (displayln "-------------------- Constant -----------------------")
  (displayln constant-variable-table)
  (displayln "-------------------- Reducible Table -----------------------")
  (displayln reducible-variable-table)
  (displayln "------------ Purely Useless Table -------------")
  (displayln purely-useless-variable-table)
  (displayln "\n\n")
|#
  (define useful-dcl-tree (remove-useless-vars func-node))
  useful-dcl-tree
  )

; Procedure Block Parser (DCL -> Validity Check)
(define (procedure-parse tree type phase)
  ...
  (match phase
    ['ANALYSIS
      ...
      ; Since in WLP4, same name function is not allowed in the grammar
    ...
    ['OPTIMIZE
       (define reduced-sub-tree (reduce-uesless-declarations tree param-objects))
       ;(println reduced-sub-tree)
       reduced-sub-tree
      ]
    [_
     (print-end "ERROR: This Code Should Not Be Reachable")
     ]))


(define (build-o-reduced-tree b-tree phase)  
  (define (generate-o-reduced-tree tree)
    (define val-list (string-split (v-obj-val (node-vobj tree))))
    
    (define (build-o-reduced-list tree-lst result)
      (if (empty? tree-lst)
          (reverse result)
          (build-o-reduced-list
           (cdr tree-lst)
           (cons (generate-o-reduced-tree (car tree-lst)) result))))
    
    (match val-list
      [(list "main" "INT" "WAIN" "LPAREN" "dcl" "COMMA" "dcl" "RPAREN"
             "LBRACE" "dcls" "statements" "RETURN" "expr" "SEMI" "RBRACE")
       (procedure-parse tree "main" phase)
       ;(node (node-vobj tree) (build-o-reduced-list (node-lst tree) empty))
       ]
      [(list "procedure" "INT" "ID" "LPAREN" "params" "RPAREN"
             "LBRACE" "dcls" "statements" "RETURN" "expr" "SEMI" "RBRACE")
       (procedure-parse tree "procedure" phase)
       ;(node (node-vobj tree) (build-o-reduced-list (node-lst tree) empty))
       ]
      [_
       (node
        (node-vobj tree)
        (build-o-reduced-list (node-lst tree) empty))
       ]))
  (define reduced-tree (generate-o-reduced-tree b-tree))
  ;(println reduced-tree)
  ;(set! parse-tree parse-tree)
  (set! parse-tree reduced-tree)
  )

; Whole Program Parser
(define (parse-elements tree phase)
  (set! derailed #f)
  ...
  (match phase
        ['ANALYSIS
         (cond
           [(equal? d-root "main")
            (procedure-parse tree d-root phase)
            (set! current-scope "")
            (set! derailed #t)
            ]
         ...
         (if (not derailed)
             (for [(subtree-node (node-lst tree))]
               (parse-elements subtree-node phase))
             (void))]
        ['OPTIMIZE
         (build-o-reduced-tree tree phase)
         ]))
...
(define (context-sensitive-analysis)
  (begin
    ...
    (generate-parse-tree)
    (parse-elements parse-tree 'ANALYSIS)
    (build-typed-parse-tree)
    (parse-elements parse-tree 'OPTIMIZE)
    ;(output-sym-table)
    ))
...
;cat foo.wlp4 | wlp4scan | wlp4parse | racket wlp4gen.rkt | cs241.linkasm | cs241.linker print.merl > foo.mips